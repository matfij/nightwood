...
export class ActionEventService {
    ...
    async startExpedition(userId: number, dto: StartExpeditionDto): Promise<DragonActionDto> {
        ...
        const action = await this.dragonActionService.startExpedition(
            dto.expeditionUid,
            dragon,
            timeReduction,
        );
    ...
    async checkExpeditions(userId: number): Promise<ExpeditionReportDto[]> {
        ...
        for (const dragon of dragons) {
            ...
            const gainedEter = await this.dragonService.awardExpeditionEter(dragon, expedition);
    ...
    async extendDragonLimit(userId: number): Promise<void> {
        ...
        const actionCost =
            ACTION_INCREASE_DRAGON_LIMIT.baseCost +
            ACTION_INCREASE_DRAGON_LIMIT.costFactor * (user.maxOwnedDragons - DRAGON_BASE_LIMIT);
