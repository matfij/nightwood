...
class DragonService:
    ...
    def gen_attributes(cls, level):
        attribute_pool = 5 + random.randint(math.floor(0.8 * level), math.floor(1.6 * level))
    ...
    def gen_skills(cls, level, nature):
        skill_pool = 1 + random.randint(math.floor(1 * level), math.floor(2.2 * level))
